...
import { useAdminDashboardBalances } from '@/hooks/api';
...
export function AdminDashboardBalances() {
  const { t } = useTranslation();
  const theme = useTheme();
  const { data, isLoading } = useAdminDashboardBalances();
  ...
  return (
    ...
    >
      ...
      <SectionHeader
        title={t('admin.dashboardBalances')}
        subtitle={t('admin.dashboardBalancesHint')}
      />
        ...
        <>
          ...
          >
            ...
            <Typography sx={{ fontSize: 12.5, fontWeight: 600, color: 'text.secondary', display: 'inline-flex', alignItems: 'center', gap: 0.5 }}>
              ...
              {t('admin.totalCurrencies', { count: ORDER.length })}
            </Typography>
          </Box>
        </>
      )}
