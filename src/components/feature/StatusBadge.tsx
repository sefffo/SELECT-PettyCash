...
const statusConfig: Record<Status, { color: string; bg: string; darkText: string; lightText: string; key: string; icon: ReactElement }> = {
  ...
  rejected: { color: '#DC2626', bg: 'rgba(220, 38, 38, 0.12)', darkText: '#FCA5A5', lightText: '#991B1B', key: 'expense.status.rejected', icon: <Cancel /> },
...
export function getStatusLabelKey(status: Status): string | undefined {
  return statusConfig[status]?.key;
}