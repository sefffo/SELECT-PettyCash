...
export default function EmployeeSettings() {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }}>
      <Box mb={3}>
        <Typography variant="h2" sx={{ color: 'text.primary' }}>{t('employee.settings')}</Typography>
        <Typography sx={{ fontSize: 15, color: 'text.secondary', mt: 0.25 }}>{t('employee.settingsSubtitle')}</Typography>
      </Box>
      ...
      <Box display="flex" flexDirection="column" gap={2.5} maxWidth={720}>
        ...
        <ChangePasswordCard
          title={t('employee.changePassword')}
          subtitle={t('employee.changePasswordSubtitle')}
          submitLabel={t('employee.changePassword')}
